...
async function fetchSensorData() {
  ...
  try {
    const { data: danData } = await supabaseAdmin
      .from('sensor_data')
      .select('*')
      .ilike('location', '%Danapur%');
    log('Danapur sensor_data:', danData);

    const { data: nagData } = await supabaseAdmin
      .from('sensor_data')
      .select('*')
      .ilike('location', '%Nagpur%');
    log('Nagpur sensor_data:', nagData);
  } catch (err) {
    log('Error: ' + err.message);
  }
  fs.writeFileSync('scripts/inspect_sensor_data.log', logContent);
}
