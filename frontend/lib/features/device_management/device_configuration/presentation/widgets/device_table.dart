...
class SearchParams {
  ...
  SearchParams(this.deviceList, this.query);
}

List<DeviceEntity> _filterDevice(SearchParams params) {
  final query = params.query.toLowerCase();
  return params.deviceList.where((device) {
    return [
      device.shortName,
      device.fullName,
      device.dataType,
...
class DeviceTable extends StatefulWidget {
  final List<DeviceEntity> items;
  final Function(DeviceEntity device) onTapEdit;
  final Function(DeviceEntity device) onTapView;
  final Function(DeviceEntity device) onTapDelete;
...
class _DeviceTableState extends State<DeviceTable> {
  ...
  @override
  void didUpdateWidget(covariant DeviceTable oldWidget) {
  ...
  void _buildDataSource() async {
    final deviceListToFilter = widget.items;

    List<DeviceEntity> filteredItems;
    if (_searchText.isEmpty) {
      filteredItems = deviceListToFilter;
    } else {
      filteredItems = await compute(
          _filterDevice, SearchParams(deviceListToFilter, _searchText));
    }
              ...
              child: Container(
                ...
                child: SfDataGridTheme(
                  ...
                  child: SfDataGrid(
                    ...
                    columns: [
                      Utils.gridColumn(label: 'Short Name'),
                      Utils.gridColumn(label: 'Full Name'),
                      Utils.gridColumn(label: 'Data Type'),
                      Utils.gridColumn(label: 'Sampling Freq'),
                      Utils.gridColumn(label: 'Status'),
                      Utils.gridColumn(label: 'Sensors'),
...
class TableDataSource extends DataGridSource {
  final List<DeviceEntity> items;
  final Function(DeviceEntity deviceItem) onTapEdit;
  final Function(DeviceEntity deviceItem) onTapView;
  final Function(DeviceEntity deviceItem) onTapDelete;
  ...
  TableDataSource({
    ...
    String emptyText = '----';
    _rows = items.map((deviceItem) {
      return DataGridRow(cells: [
        DataGridCell<String>(
            columnName: 'Short Name', value: deviceItem.shortName ?? emptyText),
        DataGridCell<String>(
            columnName: 'Full Name', value: deviceItem.fullName ?? emptyText),
        DataGridCell<String>(
            columnName: 'Data Type', value: deviceItem.dataType ?? emptyText),
        DataGridCell<String>(
            columnName: 'Sampling Freq',
            value: (deviceItem.frequency ?? '').toString()),
        DataGridCell<String>(
            columnName: 'Status',
            value: DeviceStatusHelper.fromId(deviceItem.isActive).name),
        DataGridCell<String>(
            columnName: 'Sensors',
            value: (deviceItem.numberOfSensors ?? 0).toString()),
        DataGridCell<String>(
            columnName: 'Created', value: deviceItem.createdAt ?? emptyText),
        DataGridCell<String>(columnName: 'Created By', value: deviceItem.createdBy ?? emptyText),
        DataGridCell<String>(
            columnName: 'Updated', value: deviceItem.updatedAt ?? emptyText),
        DataGridCell<String>(
            columnName: 'Updated By', value: deviceItem.updatedBy ?? emptyText),
